...
class HistoryService:
    ...
    async def iter_replies_to(
        ...
        target_message: Union[Message, int],
        vicinity: int = 500,
    ) -> AsyncGenerator[Message, None]:
        target_message_id: int = (
            target_message
            if isinstance(target_message, int)
            else target_message.message_id
        )
        ...
        async for m in cast(AsyncGenerator[Message, None], history_gen):
            ...
            if (
                m.reply_to_message
                and m.reply_to_message.message_id == target_message_id
            ):
                yield m

    async def get_reply_counts(
        self, chat_id: Union[int, str], lookback: int = 3000
    ) -> Counter:
        counter: Counter = Counter()

        async for m in cast(
            AsyncGenerator[Message, None],
            self.client.iter_history(chat_id, limit=lookback),
        ):
