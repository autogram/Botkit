...
# TODO: Should use boltons.MultiReplace(object)
...
ORDERED = OrderedDict(
    [
        ...
        (
            r"(?:(?<=\W)|^)-(.+?)-(?:(?=\W)|$)",
            (lambda g: mdformat.strikethrough(g[0]), re.DOTALL),
        ),  # strikethrough
        (
            r"(?:(?<=\W)|^)#(.+?)#(?:(?=\W)|$)",
            (lambda g: mdformat.smallcaps(g[0]), re.DOTALL),
        ),  # smallcaps
...
STATIC = {
    ...
    r"\blorem([0-9]?)\b": lambda g: "\n".join(
        lorem.sentence() for _ in range(int(g[0]) if g[0] else 1)
    ),
                ...
                def substitute(match):
                    ...
                    def escape_or_replace(m):
                        try:
                            ...
                            raise ValueError(
                                f"There is no capturing group defined in the pattern {pattern}, "
